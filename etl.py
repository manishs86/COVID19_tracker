...
def load_time_series(source='web', update='manual'):
    if source == 'web':
        ...
        while not current_data:
            try:
                confirmed_us = save_from_web(
                    r'{}confirmed_US.csv'.format(prepend))
                print('confirmed_us      ', confirmed_us.columns[-1])
                confirmed_global = save_from_web(
                    r'{}confirmed_global.csv'.format(prepend))
                ...
                deaths_global = save_from_web(
                    r'{}deaths_global.csv'.format(prepend))
                print('deaths_global     ', deaths_global.columns[-1])
                recovered_global = save_from_web(
                    r'{}recovered_global.csv'.format(prepend))
        ...
        confirmed_us.to_csv(
            'data/raw/time_series_covid19_confirmed_US.csv', index=False)
        confirmed_global.to_csv(
            'data/raw/time_series_covid19_confirmed_global.csv', index=False)
        deaths_us.to_csv(
            'data/raw/time_series_covid19_deaths_US.csv', index=False)
        deaths_global.to_csv(
            'data/raw/time_series_covid19_deaths_global.csv', index=False)
        recovered_global.to_csv(
            'data/raw/time_series_covid19_recovered_global.csv', index=False)

    elif source == 'folder':
        confirmed_us = pd.read_csv(
            'data/raw/time_series_covid19_confirmed_US.csv')
        confirmed_global = pd.read_csv(
            'data/raw/time_series_covid19_confirmed_global.csv')
    ...
    confirmed_canada = confirmed_global_melt[confirmed_global_melt['Country/Region']
                                             == 'Canada'].groupby('date', as_index=False).agg({'Confirmed': 'sum'})
    ...
    confirmed_canada = confirmed_canada[[
        'Country/Region', 'Province/State', 'Latitude', 'Longitude', 'date', 'Confirmed', 'Admin2']]
    ...
    deaths_canada = deaths_global_melt[deaths_global_melt['Country/Region']
                                       == 'Canada'].groupby('date', as_index=False).agg({'Deaths': 'sum'})
    ...
    deaths_canada = deaths_canada[[
        'Country/Region', 'Province/State', 'Latitude', 'Longitude', 'date', 'Deaths', 'Admin2']]
    ...
    confirmed_deaths_global.loc[confirmed_deaths_global['Country/Region']
                                == 'Syria', ['Latitude']] = 34.802075
    confirmed_deaths_global.loc[confirmed_deaths_global['Country/Region']
                                == 'Syria', ['Longitude']] = 38.996815

    confirmed_deaths_global.loc[confirmed_deaths_global['Country/Region']
                                == 'Mozambique', ['Latitude']] = -18.6657
    confirmed_deaths_global.loc[confirmed_deaths_global['Country/Region']
                                == 'Mozambique', ['Longitude']] = 35.5296

    confirmed_deaths_global.loc[confirmed_deaths_global['Country/Region']
                                == 'Timor-Leste', ['Latitude']] = -8.8742
    confirmed_deaths_global.loc[confirmed_deaths_global['Country/Region']
                                == 'Timor-Leste', ['Longitude']] = 125.7275
    ...
    # fix some mismatched coordinates
    recovered_global_melt.loc[recovered_global_melt['Country/Region']
                              == 'Syria', ['Latitude']] = 34.802075
    recovered_global_melt.loc[recovered_global_melt['Country/Region']
                              == 'Syria', ['Longitude']] = 38.996815
    ...
    df.loc[(df['Country/Region'] == 'US') &
           (df['Recovered'].notna()), 'Province/State'] = 'Recovered'
    ...
    df = df.sort_values(by=['date', 'Country/Region',
                            'Province/State', 'Admin2']).reset_index(drop=True)
...
def worldwide(data):
    ...
    df = data.groupby(['date', 'Country/Region'], as_index=False).agg({'Latitude': 'mean',
                                                                       ...
                                                                       'Recovered': 'sum',
                                                                       'Active': 'sum'})
    df['share_of_last_week'] = 100 * (df['Confirmed'] - df.groupby(
        'Country/Region')['Confirmed'].shift(7, fill_value=0)) / df['Confirmed']
    df['share_of_last_week'] = df['share_of_last_week'].replace(
        [np.inf, -np.inf], np.nan).fillna(0)
    df.loc[df['share_of_last_week'] < 0, 'share_of_last_week'] = 0
    df['percentage'] = df['share_of_last_week'].apply(
        lambda x: '{:.1f}'.format(x))
    df = df[['date', 'Country/Region', 'Latitude', 'Longitude', 'Confirmed',
             'Deaths', 'Recovered', 'Active', 'share_of_last_week', 'percentage']]
...
def us(data):
    ...
    df = df.groupby(['date', 'Province/State'], as_index=False).agg({'Confirmed': 'sum',
                                                                     ...
                                                                     'Recovered': 'sum',
                                                                     'Active': 'sum'})
    df = df.merge(pd.read_csv('data/geo_us.csv'),
                  left_on='Province/State', right_on='Province/State', how='left')
...
def global_population():
    ...
    pop_global = pop_global[(pop_global['Variant'] == 'Medium') & (
        pop_global['Time'] == 2020)][['Location', 'PopTotal']].reset_index(drop=True)
...
def china_population(pop_global):
    ...
    # 2018 data
    pop_china = pd.read_csv('data/AnnualbyProvince.csv',
                            header=3, encoding='ISO-8859-1')
    ...
    pop_china.loc[31] = ['Hong Kong', pop_global[pop_global['region']
                                                 == 'China, Hong Kong SAR'].squeeze()['population']]
    pop_china.loc[32] = ['Macau', pop_global[pop_global['region']
                                             == 'China, Macao SAR'].squeeze()['population']]
    return pop_china


def population_to_worldwide(df, pop_global):
    df = pd.merge(df, pop_global, left_on='Country/Region',
                  right_on='region', how='left').drop(columns='region')

    # correct mismatched country names
    df.loc[df['Country/Region'] == 'Bolivia', ['population']] = pop_global.loc[pop_global['region']
                                                                               == 'Bolivia (Plurinational State of)'].squeeze()['population']
    df.loc[df['Country/Region'] == 'Brunei', ['population']
           ] = pop_global.loc[pop_global['region'] == 'Brunei Darussalam'].squeeze()['population']
    df.loc[df['Country/Region'] == 'Burma', ['population']
           ] = pop_global.loc[pop_global['region'] == 'Myanmar'].squeeze()['population']
    df.loc[df['Country/Region'] == 'Congo (Brazzaville)', ['population']
           ] = pop_global.loc[pop_global['region'] == 'Congo'].squeeze()['population']
    df.loc[df['Country/Region'] == 'Congo (Kinshasa)', ['population']] = pop_global.loc[pop_global['region']
                                                                                        == 'Democratic Republic of the Congo'].squeeze()['population']
    df.loc[df['Country/Region'] == "Cote d'Ivoire", ['population']
           ] = pop_global.loc[pop_global['region'] == "Côte d'Ivoire"].squeeze()['population']
    df.loc[df['Country/Region'] == 'Iran', ['population']] = pop_global.loc[pop_global['region']
                                                                            == 'Iran (Islamic Republic of)'].squeeze()['population']
    # source: https://data.worldbank.org/country/kosovo
    df.loc[df['Country/Region'] == 'Kosovo', ['population']] = 1845000
    df.loc[df['Country/Region'] == 'South Korea', ['population']
           ] = pop_global.loc[pop_global['region'] == 'Republic of Korea'].squeeze()['population']
    df.loc[df['Country/Region'] == 'Laos', ['population']] = pop_global.loc[pop_global['region']
                                                                            == "Lao People's Democratic Republic"].squeeze()['population']
    df.loc[df['Country/Region'] == 'Moldova', ['population']
           ] = pop_global.loc[pop_global['region'] == 'Republic of Moldova'].squeeze()['population']
    df.loc[df['Country/Region'] == 'Russia', ['population']
           ] = pop_global.loc[pop_global['region'] == 'Russian Federation'].squeeze()['population']
    df.loc[df['Country/Region'] == 'Taiwan', ['population']] = pop_global.loc[pop_global['region']
                                                                              == 'China, Taiwan Province of China'].squeeze()['population']
    df.loc[df['Country/Region'] == 'Tanzania', ['population']] = pop_global.loc[pop_global['region']
                                                                                == 'United Republic of Tanzania'].squeeze()['population']
    df.loc[df['Country/Region'] == 'Venezuela', ['population']] = pop_global.loc[pop_global['region']
                                                                                 == 'Venezuela (Bolivarian Republic of)'].squeeze()['population']
    df.loc[df['Country/Region'] == 'Vietnam', ['population']
           ] = pop_global.loc[pop_global['region'] == 'Viet Nam'].squeeze()['population']
    df.loc[df['Country/Region'] == 'Syria', ['population']
           ] = pop_global.loc[pop_global['region'] == 'Syrian Arab Republic'].squeeze()['population']
    df.loc[(df['Country/Region'] == 'US'), ['population']] = pop_global.loc[pop_global['region']
                                                                            == 'United States of America'].squeeze()['population']
...
def population_to_us(df, pop_us):
    df = pd.merge(df, pop_us, left_on='Country/Region',
                  right_on='region', how='left').drop('region', axis=1)
    df.loc[df['Country/Region'] == 'Recovered', ['population']
           ] = pop_us[pop_us['region'] == 'United States'].squeeze()['population']
    df['population'] = df['population'] / 100000
    return df
...
def population_to_china(df, pop_china):
    df = pd.merge(df, pop_china, left_on='Country/Region',
                  right_on='region', how='left').drop('region', axis=1)
    df['population'] = df['population'] / 100000
    return df
